#!/usr/bin/env node
/**
 * Sarıkırmızı ring (sarı–kırmızı hattın 2. parçası): durak sırası + OSRM polyline.
 * Sarı ring’den hemen sonra rings.json’a yerleşir.
 */
...
const stops = []
for (const id of KIRMIZI_STOP_ORDER) {
...
try {
  ...
  console.error('Sarıkırmızı ring OSRM nokta sayısı:', polyline.length)
} catch (e) {
...
const kirmiziRing = {
  id: 'kirmizi',
  name: 'Sarıkırmızı ring',
  color: '#dc2626',
  polyline,
  stops,
}

const existing = raw.rings.findIndex((r) => r.id === 'kirmizi')
if (existing >= 0) {
  raw.rings.splice(existing, 1)
}

const sariIdx = raw.rings.findIndex((r) => r.id === 'sari')
const insertAt = sariIdx >= 0 ? sariIdx + 1 : 0
raw.rings.splice(insertAt, 0, kirmiziRing)

fs.writeFileSync(ringsPath, JSON.stringify(raw, null, 2) + '\n', 'utf8')
console.error('Sarıkırmızı ring yazıldı (sarıdan sonra):', ringsPath)
