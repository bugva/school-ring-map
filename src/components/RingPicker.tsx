...
type Props = {
  rings: Ring[]
  /** Bu id’lerde renk soluk (bugün çalışmıyor / bitti / gri hafta içi) */
  lowFocusRingIds: ReadonlySet<string>
  hasMetro: boolean
  metroLowFocus: boolean
      ...
      <div className="ring-picker__chips" role="listbox" aria-label="Ring listesi">
        {hasMetro ? (
          <button
            ...
            className={`ring-picker__chip${metroSelected ? ' ring-picker__chip--selected' : ''}${metroLowFocus && !metroSelected ? ' ring-picker__chip--low-focus' : ''}`}
        ...
        {rings.map((ring) => {
          ...
          const lowFocus = lowFocusRingIds.has(ring.id)
          return (
            <button
              key={ring.id}
              type="button"
              ...
              className={`ring-picker__chip${selected ? ' ring-picker__chip--selected' : ''}${lowFocus && !selected ? ' ring-picker__chip--low-focus' : ''}`}
